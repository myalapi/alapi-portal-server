...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        logger.log({
            level: "info",
            message: `Link portal: Create merchant API, ip: ${IP.address()} userId: ${userId} URL: ${req.protocol
        ...
        logger.log({
            level: "error",
            message: `Link portal: Create merchant API, ip: ${IP.address()} error: ${error.message
                } userId: ${userId} URL: ${req.protocol}://${req.get("host")}${req.originalUrl
                }`,
        });
